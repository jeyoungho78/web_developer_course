...
public class MybatisLogInterceptor implements Interceptor{
	...
	public Object intercept(Invocation invocation) throws Throwable {
        ...
        if(param == null){				// 파라미터가 아무것도 없을 경우
        	...
        	for(ParameterMapping mapping : paramMapping){
        	    ...
        	    if(null != propValue && !"".equals(propValue)){
        	    	if(propValue.indexOf("__frch_item") > -1){
        	    		sql = sql.replaceFirst("\\?", "[" + propValue + "]" + "/**p*/");
        	    		continue;
        	    	}
        	    	...
        	    	if(value instanceof String){			// SQL의 ? 대신에 실제 값을 넣는다. 이때 String 일 경우는 '를 붙여야 하기땜에 별도 처리
        	    		sql = sql.replaceFirst("\\?", "'" + value + "'" + "/**p*/");
        	    	}else{
        	    		sql = sql.replaceFirst("\\?", String.valueOf(value) + "/**p*/");
        	    	}
        	    }
        	...
        	for(ParameterMapping mapping : paramMapping){
        		...
        		sql = sql.replaceFirst("\\?", "'" + field.get(param) + "'" + "/**p*/");
        	    }else{
        		sql = sql.replaceFirst("\\?", field.get(param).toString() + "/**p*/");
        	    }
        ...
        if(!"real".equals(serverMode)){
        	// returnObj를 List로 바라볼수 있다면
        	if(returnObj instanceof List){
        		for(Object obj: (List)returnObj){
        			logger.debug(">>>>> : " + obj);
        		}
        	}else{
        		logger.debug(">>>>> : " + returnObj.toString());
        	}
        }
        logger.debug("=====================================================================");
        return returnObj; // 쿼리 실행
